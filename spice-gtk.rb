...
@@ -350,9 +384,17 @@ static void spice_gtk_session_set_property(GObject      *gobject,
     case PROP_SESSION:
...
+
+        g_object_get(s->session, "disable-clipboard", &disable_clipboard, NULL);
+        if (disable_clipboard && g_value_get_boolean(value)) {
...
@@ -858,6 +900,19 @@ static void clipboard_get(GtkClipboard *clipboard,
...
+
     if (s->clipboard_release_delay[selection]) {
         ...
         return;
@@ -909,6 +964,152 @@ static void clipboard_clear(GtkClipboard *clipboard, gpointer user_data)
...
+
 static gboolean clipboard_grab(SpiceMainChannel *main, guint selection,
                                ...
                                gpointer user_data)
@@ -959,6 +1160,29 @@ static gboolean clipboard_grab(SpiceMainChannel *main, guint selection,
         return TRUE;
     }
...
@@ -34,6 +35,7 @@ static char *usbredir_redirect_on_connect = NULL;
...
@@ -179,6 +181,8 @@ GOptionGroup* spice_get_option_group(void)
...
@@ -319,6 +323,8 @@ void spice_set_session_option(SpiceSession *session)
...
index b4919a4..11fb6b4 100644
--- a/src/spice-session-priv.h
+++ b/src/spice-session-priv.h
@@ -56,6 +56,7 @@ void spice_session_abort_migration(SpiceSession *session);
...
index bb3c6cd..76031e1 100644
--- a/src/spice-session.c
+++ b/src/spice-session.c
@@ -89,6 +89,9 @@ struct _SpiceSessionPrivate {
...
+    PROP_DISABLE_CLIPBOARD,
     ...
     PROP_COLOR_DEPTH,
@@ -669,6 +673,9 @@ static void spice_session_get_property(GObject    *gobject,
     case PROP_USBREDIR:
...
+        break;
     case PROP_INHIBIT_KEYBOARD_GRAB:
         ...
         break;
@@ -813,6 +820,9 @@ static void spice_session_set_property(GObject      *gobject,
     case PROP_USBREDIR:
...
+        break;
     case PROP_INHIBIT_KEYBOARD_GRAB:
         ...
         break;
@@ -1234,6 +1244,24 @@ static void spice_session_class_init(SpiceSessionClass *klass)
...
+     *
+     * Whether clipboard sharing was disabled on the command line. This
+     * property is read-only, and set through a method so that applications
+     * using the library cannot override what the user set on the command
+     * line.
+     *
+     * Since: 0.35
+     **/
...
+
      ...
      *
@@ -2834,6 +2862,14 @@ gboolean spice_session_get_smartcard_enabled(SpiceSession *session)
